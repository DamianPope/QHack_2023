# -*- coding: utf-8 -*-
"""
Created on Tue Feb 28 02:39:01 2023
...
def circuit_left():
    ...
    """
    #code *before* bit flip in bit flip error correction
    qml.CNOT(wires=[0,1])

    #execute CNOT between qubits 0 & 2 without directly connecting them
    qml.SWAP(wires=[0,1])
    qml.CNOT(wires=[1,2])
    qml.SWAP(wires=[0,1])
...
def circuit_right():
    ...
    description. Simply place the necessary operations, you do not have to return anything.
    """
    #code *after* bit flip in bit flip error correction
    qml.CNOT(wires=[0,1])
    
    #execute between qubits 0 & 2 without directly connecting them
    qml.SWAP(wires=[0,1])
    qml.CNOT(wires=[1,2])
    qml.SWAP(wires=[0,1])
    
    #execute qml.Toffoli(wires=[1,2,0]) without directly connecting qubits 0 & 2
    #The gates below come from the Wikipedia page on the Toffoli gate: https://en.wikipedia.org/wiki/Toffoli_gate
    ...
    #move state of qubit 0 to qubit 2
    qml.SWAP(wires=[0,1])
    qml.SWAP(wires=[1,2])
